...
class SignMainFragment : Fragment() {
    override fun onCreateView(
        ...
        viewModel.goToEmail.observe(viewLifecycleOwner, Observer {
            if(it) {
                findNavController().navigate(SignMainFragmentDirections.actionSignMainFragmentToSigninFragment())
                viewModel.doneGoToEmail()
            }
        })
